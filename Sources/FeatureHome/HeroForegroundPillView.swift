...
/// selected-highlight the same way the SwiftUI `heroPill` does: a translucent
/// capsule normally, a bright white capsule with dark glyph and a slight lift when
/// it is the selected pill and the hero holds focus.
///
/// Deliberate POC visual differences from the SwiftUI hero (documented): the idle
/// background is a flat translucent capsule rather than tvOS Liquid Glass, and the
/// resume/download progress is a simple inline bar.
final class HeroForegroundPillView: UIView {
    ...
    func configure(_ pill: HeroForegroundModel.Pill, selected: Bool) {
        ...
        backgroundColor = selected ? .white : UIColor.white.withAlphaComponent(0.18)
        setNeedsLayout()

        // Lift only the scale animates as selection moves (matches SwiftUI 0.16 easeOut).
    ...
    override func layoutSubviews() {
        ...
        if !glyphView.isHidden {
            ...
            x += glyphSize + (textLabel.isHidden ? 0 : glyphTextGap)
        }
        if !textLabel.isHidden {
        ...
        if !progressTrack.isHidden {
            let trackWidth: CGFloat = 90
            let trackHeight: CGFloat = 6
            let tx = textLabel.isHidden ? x : (textLabel.frame.maxX - trackWidth)
            progressTrack.frame = CGRect(x: max(hPadding, tx), y: bounds.maxY - vPadding + 2, width: trackWidth, height: trackHeight)
            progressTrack.layer.cornerRadius = trackHeight / 2
            progressFill.frame = CGRect(x: 0, y: 0, width: trackWidth * progressFraction, height: trackHeight)
