...
public final class NativeVideoEngine: VideoEngine {
    ...
    #endif
    #if canImport(UIKit)
    /// A single, stable controller fed by whichever `AVPlayer` is live, so a
    /// transcode-fallback swap re-points the existing controller instead of
    /// forcing the SwiftUI layer to rebuild it.
    @ObservationIgnored private var playerViewController: AVPlayerViewController?
    #endif
    ...
    public func makePlayerViewController() -> UIViewController {
        if let existing = playerViewController { return existing }
