...
enum OnlineTrailerSource {
    ...
    static func trailers(for item: MediaItem, resolver: TMDbArtworkResolver = .shared) async -> [MediaItem] {
        ...
        let videoIDs = await resolver.trailerVideoIDs(title: q.title, year: q.year, isTV: q.isTV, tmdbID: q.tmdbID)
    ...
    static func query(for item: MediaItem) -> (title: String, year: Int?, isTV: Bool, tmdbID: String?)? {
        switch item.kind {
        case .movie, .video:
            return (item.title, item.productionYear, false, item.providerIDs["Tmdb"])
        case .series:
            return (item.title, nil, true, item.providerIDs["Tmdb"])
