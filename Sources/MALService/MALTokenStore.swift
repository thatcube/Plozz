...
public final class KeychainMALTokenStore: MALTokenStoring, @unchecked Sendable {
    ...
    public init(service: String = "com.plozz.app.tokens", account: String = "mal.oauth", namespace: String? = nil) {
    ...
    public func load() -> MALTokens? {
        ...
        return try? JSONDecoder().decode(MALTokens.self, from: data)
    }
    ...
    public func save(_ tokens: MALTokens) throws {
        ...
        guard status == errSecSuccess else { throw MALTokenStoreError.unexpectedStatus(status) }
    }

    public func clear() throws {
        let status = SecItemDelete(baseQuery(account: currentAccount()) as CFDictionary)
        guard status == errSecSuccess || status == errSecItemNotFound else {
            throw MALTokenStoreError.unexpectedStatus(status)
        }
    }
}
