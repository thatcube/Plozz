...
/// Minimal random-access byte source the Matroska sampler pulls from. Unit tests
/// use an in-memory `Data`-backed source so the EBML byte-shape parsing is
/// exercised without any network or libavformat.
...
/// until a caller wires it into the background discovery path behind a flag.
///
/// ROLE (coordinator-granted): the **background full-timeline walk** engine for
/// Track C's no-Cues populator. The coordinator scoped the swarm to B6's resync'd
/// C parser (`plozz_remux_kf_probe_*`) for live per-seek probes, but granted this
/// sampler for the *different* access pattern of "walk the WHOLE timeline once,
/// off the playback path" — a single sequential cluster-size-skip walk beats N
/// independent per-seek probes for enumerating every keyframe. It is a cold
/// background path on rare no-Cues titles only; the deliberate 2nd-EBML-reader
/// cost is accepted for that one job. Its output is a `KeyframeTable` that flows
/// through Track A's provider into B7's engine — it never serves, muxes, or
/// resolves live seeks. Currently PARKED in the test target (already exists /
/// tested / inert); it relocates to `Sources/LocalRemux/` and gets wired the
/// moment the coordinator un-holds the background scan (held pending B6's
/// "found N CuePoints" result sizing the no-Cues class).
///
/// HARVEST SOURCE at un-hold: B5 `thatcube-remux-desync-stutter-fix` @ **69f5238**
/// (142 tests) — re-harvest the CURRENT sampler from there rather than this parked
/// copy, because B5's Cues reader now shares `parseInit` with the no-Cues walk in
/// the same parser. That gives ONE parser covering BOTH paths: the no-Cues
/// background full-walk AND the Cues fast-path both emit `KeyframeTable.times`.
