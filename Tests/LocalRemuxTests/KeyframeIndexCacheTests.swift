...
final class KeyframeIndexCacheTests: XCTestCase {
    ...
    func testStoreLoadRoundTrip() {
        ...
        cache.store(key: key, size: 12345, duration: 24.0, target: 6.0, times: boundaries)
    ...
    func testLoadAcceptsDurationWithinTolerance() {
        let (cache, dir) = makeTempCache()
        defer { cleanup(dir) }
        cache.store(key: "k", size: 1000, duration: 60.0, target: 6, times: [0, 6, 60])
    ...
    func testLoadRejectsTooFewBoundaries() {
        ...
        cache.store(key: "k", size: 1, duration: 1, target: 6, times: [0])
    ...
    func testLoadRejectsNonMonotonicBoundaries() {
        ...
        cache.store(key: "k", size: 1, duration: 30, target: 6, times: [0, 12, 6, 30])
        XCTAssertNil(cache.load(key: "k", expectedSize: 1, expectedDuration: 30))
    }

    // MARK: - Raw times persistence

    /// The cache stores the fundamental keyframe **times** (PTS), not one cadence's
    /// baked cut points, so the planner can re-derive any cadence on the next open.
    func testRawKeyframeTimesRoundTrip() {
        let (cache, dir) = makeTempCache()
        defer { cleanup(dir) }
        let times: [Double] = [0, 6.04, 11.39, 24.65, 30.0]
        cache.store(key: "k", size: 999, duration: 30.0, target: 6, times: times)
        let loaded = cache.load(key: "k", expectedSize: 999, expectedDuration: 30.0)
        XCTAssertEqual(loaded, times)
    }

    /// A v1 (legacy `boundaries`) payload must be rejected after the v2 bump so a
    /// stale-format entry can never be restored.
    func testRejectsOlderPayloadVersion() {
        let (cache, dir) = makeTempCache()
        defer { cleanup(dir) }
        let key = "legacy"
        let legacy = """
        {"v":1,"size":999,"duration":30,"target":6,"boundaries":[0,6,30]}
        """
        try? legacy.write(to: dir.appendingPathComponent("\(key).json"),
                          atomically: true, encoding: .utf8)
        XCTAssertNil(cache.load(key: key, expectedSize: 999, expectedDuration: 30))
    }
}
#endif
