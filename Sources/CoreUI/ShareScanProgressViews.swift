...
public struct ShareScanStatusRow: View {
    ...
    public var body: some View {
        ...
        .accessibilityLabel(
            displayName + Text(verbatim: ", ") + detailLine
        )
    }
    ...
    private var detailLine: Text {
        ...
        return Text(phase)
            + Text(verbatim: " · ")
            + scanProgressDetailText(progress)
    }

    /// A named share is provider content; the safety-net fallback is app copy.
...
public struct ShareScanStatusCard: View {
    ...
    private var headerTitle: LocalizedStringResource {
        "Updating \(states.count) libraries"
    }
}
