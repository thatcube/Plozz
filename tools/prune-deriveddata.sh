...
#   * ~/Library/Caches/plozz-mpv/mpv  — the prebuilt mpv/FFmpeg codec cache
#     (~78 MB) that tools/setup-mpv.sh clones into each worktree. The slow
#     (~9-min) mpv build is shared across ALL branches via this cache and is
#     never regenerated by cleaning DerivedData. This script lives in a
#     different tree and can never delete it.
...
#   --all            Remove every project DerivedData folder (full reset). The
#                    shared mpv cache still survives; next build is a cold
#                    compile. Recently-active folders are still skipped.
#
# FLAGS
