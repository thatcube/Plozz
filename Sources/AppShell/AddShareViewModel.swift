    ...
    func connectManualHost() {
        ...
        detectTask = Task { [routeDetector] in
            ...
            case .success(.smb(let host, let port)):
                ...
                self.enterShareStep()
            case .success(.nfs):
                // NFS onboarding is owned by the unified media-share flow, not
                // this legacy SMB/WebDAV screen. Detection here still proves the
                // NFSClaimant routes nfs:// correctly through the registry.
