...
final class ShareScanStatusModelTests: XCTestCase {
    ...
    func testDisplayNameNeverBlankWhenEnrichBeatsANamedScanStart() {
        let model = ShareScanStatusModel()
        model.enrichStarted(shareID: "s1", total: 10)
        // CoreModels retains the empty provider name as facts. CoreUI supplies
        // the localized "Media library" fallback at render time.
        XCTAssertEqual(model.state(forShareID: "s1")?.name, "")
    }
