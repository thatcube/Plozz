...
/// expects.
private func makeHeroFeaturedProvider(
    seer: SeerService,
...
private func makeHeroFeaturedStatusProvider(
    seer: SeerService,
    hideWatched: Bool
) -> FeaturedContentProviding {
    { limit in
...
enum HeroCandidatePool {
    ...
    /// is disabled.
    static func requestLimit(finalLimit: Int, hideWatched: Bool) -> Int {
...
/// Folds live provider watch history onto Seerr's discovery-only items. Identity
/// index membership avoids searches; bounded item-detail fetches supply current
/// profile watch state without allowing multi-server setups to flood networking.
enum HeroCandidateWatchStateEnricher {
