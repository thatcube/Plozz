...
public enum SeriesUpcoming {
        ...
        return schedule
            ...
            .map { index, pair in
                ...
                return MediaItem(
                    ...
                    // The nearest release benefits from a precise clock time;
                    // repeating it on every later placeholder makes the rail
                    // harder to scan.
                    showsScheduledReleaseTime:
                        index == 0 && episode.datePrecision == .dateAndTime
                )
            }
    }
    ...
    public static func heroLine(
        ...
        if isSeasonPremiere, isFullSeasonDrop(schedule, premiere: nextEpisode) {
            ...
            if sameYear {
                return "Full season releases \(nextEpisode.airDate, format: .dateTime.month(.abbreviated).day())"
            }
        ...
        if let cadence,
           ...
           !isSeasonPremiere, !isWithinAWeek(nextEpisode.airDate, from: now, calendar: calendar) {
            ...
            guard let representative = representativeDate(
                forWeekday: weekday,
                after: now,
                calendar: calendar
            ) else { return nil }
            return "New episodes on \(representative, format: .dateTime.weekday(.wide))"
        }

        let days = calendar.dateComponents(
            [.day],
            from: calendar.startOfDay(for: now),
            to: calendar.startOfDay(for: nextEpisode.airDate)
        ).day ?? 0
        if isSeasonPremiere {
            switch days {
            ...
            default:
                ...
                return "New season \(nextEpisode.airDate, format: .dateTime.month(.abbreviated).day().year())"
            }
        } else {
            switch days {
            ...
            default:
                ...
                return "New episode \(nextEpisode.airDate, format: .dateTime.month(.abbreviated).day().year())"
            }
        }
    }
